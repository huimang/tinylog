...
viewer 会保持运行，像一个轻量级的 vim 风格浏览器。展示区分为左右两栏：左边是蓝色的逻辑日志行号，右边是内容区。单条日志可能因为宽度限制或自身换行而展示成多行，但仍只对应一个逻辑序号。当前焦点行一开始会出现在内容区第一行；当它移动到内容区物理第 5 行后，展示区域才开始向上滚动，并尽量把焦点固定在那里直到接近文件末尾。
