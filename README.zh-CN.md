...
TinyLog 发布版本遵循 `MAJOR.MINOR.PATCH`。

- 小优化和 bug 修复：递增 `PATCH`
- 向后兼容的新功能：递增 `MINOR`，并将 `PATCH` 置为 `0`
- 不兼容或升级幅度较大的变更：递增 `MAJOR`，并将 `MINOR`、`PATCH` 置为 `0`
...
可以使用下面的脚本构建当前版本的二进制包：

```bash
scripts/package-release.sh
```

对于 Darwin arm64 上的 `v0.1.0`，产物路径为：
