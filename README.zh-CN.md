...
2. **Rust Viewer**：用于把明文日志转换成 `.tog` 专有日志文件，并以类似 `vim` 的方式打开、浏览、搜索和定位
...
| `tinylog-viewer` | 用于把明文日志转换成 `.tog`，并浏览专有日志文件的 Rust 客户端 |
...
- **Rust Viewer**：位于 `tinylog-viewer` 的独立 Cargo 工程，同时负责 `.tog` 转换与交互式浏览
...
转换器会把这个时间文本按 **UTC 日历值** 解释，把第一个 `[LEVEL]` 标记提取到独立的 1 字节级别字段中，并把这段级别文本从最终存储的消息内容里移除；viewer 再用持久化的级别和 UTC 偏移时间恢复展示内容。
