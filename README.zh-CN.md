...
viewer 会保持运行，像一个轻量级的 vim 风格浏览器。展示区分为左右两栏：左边是蓝色的逻辑日志行号，右边是内容区，行号右侧会预留一个淡橙色的单字符位置来标记当前焦点行。单条日志可能因为宽度限制或自身换行而展示成多行，但仍只对应一个逻辑序号。当前焦点行会先在可视区域内自由移动，只有继续向上或向下移动会把它推出屏幕边界时，展示区域才会开始滚动。
